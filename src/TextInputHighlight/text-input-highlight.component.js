...
export default defineComponent({
  props: ['tagCssClass', 'tags', 'textInputElement', 'textInputValue'],
  emits: ['tagClick', 'tagMouseEnter', 'tagMouseLeave'],
  setup(props, ctx) {
    const { proxy: vm } = getCurrentInstance();
    const { tagCssClass, tags, textInputElement, textInputValue } =
      toRefs(props);
    ...
    function refresh() {
      ...
      const containerStyle = highlightElementContainerStyle.value;
      styleProperties.forEach((prop) => {
        containerStyle[prop] = computedStyle[prop];
      });
      highlightElementContainerStyle.value = { ...containerStyle };
    }
        ...
        if (!isDestroyed.value) {
          ...
          textareaEventListeners.value = [
            ...
            renderer_listen(textInputElement.value, 'scroll', () => {
              ...
              highlightTagElements.value = highlightTagElements.value.map(
                (tag) => {
            ...
            renderer_listen(textInputElement.value, 'mouseup', () => {
              refresh();
            }),
          ];
    ...
    function addTags() {
      const _textInputValue =
        typeof textInputValue.value !== 'undefined'
      ...
      [...tags.value]
        ...
        .forEach((tag) => {
          ...
          if (tagContents.length === expectedTagLength) {
            const previousIndex =
              prevTags.length > 0
                ? prevTags[prevTags.length - 1].indices.end
                : 0;
            const before = _textInputValue.slice(
              previousIndex,
              tag.indices.start
            );
    ...
    function handleTextareaMouseEvent(event, eventName) {
